...
void *idcreatorConfigInit(SpxLogDelegate *log,err_t *err){/*{{{*/
    ...
    c->waitting = 10;
    c->threadCountOfNetworkModule = 4;
    c->threadCountOfServiceModule = 8;
...
void idcreatorConfigParser(string_t line,void *config,err_t *err){/*{{{*/
    ...
    if(0 == spx_string_casecmp(*kv,"threadCountOfNetworkModule")){
        if(1 == count){
            SpxLogFmt1(c->log,SpxLogWarn,\
                    "network module thread size use default:%d.",\
                    c->threadCountOfNetworkModule);
        } else {
            u32_t threadCountOfNetworkModule = strtol(*(kv + 1),NULL,10);
            if(ERANGE == threadCountOfNetworkModule) {
                SpxLog1(c->log,SpxLogError,\
                        "bad the configurtion item of threadCountOfNetworkModule.");
                goto r1;
            }
            c->threadCountOfNetworkModule = threadCountOfNetworkModule;
        }
        goto r1;
    }

    //threadCountOfServiceModule
    if(0 == spx_string_casecmp(*kv,"threadCountOfServiceModule")){
        if(1 == count){
            SpxLogFmt1(c->log,SpxLogWarn,\
                    "threadCountOfServiceModule use default:%d.",c->threadCountOfServiceModule);
        } else {
            u32_t threadCountOfServiceModule = strtol(*(kv + 1),NULL,10);
            if(ERANGE == threadCountOfServiceModule) {
                SpxLog1(c->log,SpxLogError,\
                        "bad the configurtion item of threadCountOfServiceModule.");
                goto r1;
            }
            c->threadCountOfServiceModule = threadCountOfServiceModule;
        }
        goto r1;
    }
