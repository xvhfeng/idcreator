...
int main(int argc,char **argv) {
    ...
    gIdcreatorNetworkModule = spx_module_new(log,
            c->threadCountOfNetworkModule,
            c->stackSize,
            idcreatorNetworkModuleListeningHandler,
            &err);
    if(NULL == gIdcreatorNetworkModule){
        SpxLog2(log,SpxLogError,err,\
    ...
    gIdcreatorServiceModule = spx_module_new(log,\
            c->threadCountOfServiceModule,\
            c->stackSize,\
            idcreatorServiceModuleListeningHandler,
            &err);
    if(NULL == gIdcreatorServiceModule){
        SpxLog2(log,SpxLogError,err,\
